...
input_data = np.log1p(np.load("./data/train.npy").T[::, 0:n_input]) / 12.
target_data= np.log1p(np.load("./data/train.npy").T[::, n_input:]) / 8.
...
a = np.hstack((np.expm1(low_band*12.0), np.expm1(high_band*8.0))).T
print np.max(a)
